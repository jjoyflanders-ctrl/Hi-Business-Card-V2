...
function applyEmployee(emp){
  ...
  const deskPhotoWrap = document.querySelector('.deskPhoto');
  if(deskPhotoWrap){ deskPhotoWrap.classList.toggle('isFallback', !emp.photo); }
  ...
  if(dimg){
    ...
    dimg.onload = () => { syncDesktopHeights();
  setTimeout(()=>{ syncDesktopHeights(); }, 50); };
    dimg.src = photoSrc;
  }
  if(mimg){
...
function syncDesktopPhotoHeight(){
  const info = document.querySelector(".deskInfo");
  const photo = document.querySelector(".deskPhoto");
  if(!info || !photo) return;

  // Match the full right-column block height (name + title + green panel + buttons)
...
function syncDesktopHeights(){
  const frame = document.querySelector(".deskFrame");
  const info  = document.querySelector(".deskInfo");
  const photo = document.querySelector(".deskPhoto");
  if(!frame || !info || !photo) return;

  // Use the info column as the source-of-truth height
  const h = info.offsetHeight;
...
/* Init */
window.addEventListener("resize", ()=>{ syncDesktopHeights(); });
