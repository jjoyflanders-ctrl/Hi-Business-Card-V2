/* Highlight Digital Contact - New Design
   - Desktop + Mobile responsive layouts
   - Employees loaded from ./employees.csv (edit that file to update people)
   - Search employees via query param ?u=slug
   - Share + Send to Phone (QR + share sheet)
   - Save to Contacts (vCard download)
*/

let EMPLOYEES = []; // loaded at runtime from employees.csv
...
/* ---------------- CSV loading ----------------
   employees.csv headers expected:
   slug,firstName,lastName,title,company,phone,phoneExt,email,website,photo
   Notes:
   - If your company contains a comma, escape it like: Highlight Industries\, Inc.
   - photo can be blank to use building fallback
*/
...
async function loadEmployees(){
  try{
    // no-store helps when GitHub Pages caches aggressively
    const res = await fetch("./employees.csv", { cache: "no-store" });
    if (!res.ok) throw new Error("Failed to fetch employees.csv");
    ...
    // Safety fallback so the page never goes blank
    EMPLOYEES = [{
...
function wireActions(){
  ...
  $("#desktopShareBtn").addEventListener("click", share);
  ...
  $("#desktopSendBtn").addEventListener("click", send);
